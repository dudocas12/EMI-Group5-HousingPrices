...
def fetch_new_batch(batch_size=100):
    ...
    # 1. 'Connect' to the API and get the data
    df_future = pd.read_csv(future_data_path)

    if len(df_future) == 0:
        print("The API has no more new houses to send!")
        return

    # 2. Grab the top 100 oldest houses (chronologically)
    df_batch = df_future.head(batch_size)
    
    # 3. Remove those 100 houses from the server so we don't get them again
    df_future = df_future.iloc[batch_size:]

    # 4. Save the new batch locally and update the server
